/**
 * Log viewer component (ESM port of legacy /assets/modules/logs-ui.js).
 *
 * Same factory contract:
 *   const viewer = createLogViewer({ showToast, logsUrl });
 *   viewer.show({ isManual, serviceId, defaultServiceId });
 *   viewer.close();
 *   viewer.toggleWrap() / togglePause() / toggleFollow() / toggleFullscreen();
 *   viewer.search(text) / clearSearch() / setLevelFilter(level);
 *   viewer.copyVisible();
 *
 * Business logic intentionally identical to legacy module — we are
 * porting structure (ESM, named imports), not behavior.
 *
 * External dependencies (still global for now):
 *   window.ConfigMateUi.{openModal, closeModal, copyText}  — see ui-core.js
 *
 * DOM contract (unchanged from legacy): expects #logs-modal, #logs-content,
 * #logs-status, #logs-meta, #logs-search-input, #logs-level-filter,
 * #btn-wrap-toggle, #btn-log-follow, #btn-log-pause, #btn-log-fullscreen,
 * #logs-search-clear, #logs-title-text to exist in index.html.
 */
